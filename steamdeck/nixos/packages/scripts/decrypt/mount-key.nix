...

{
    options.ethorbit.steamdeck.packages.decrypt.mount-key = mkOption {
        ...
        default = (pkgs.writeShellScript "script" ''
            ...
            echo "179:0 150" | ${coreutils-full}/bin/tee -a /sys/fs/cgroup/blkio/blkio.throttle.write_iops_device
            echo "179:0 20000000" | ${coreutils-full}/bin/tee -a /sys/fs/cgroup/blkio/blkio.throttle.write_bps_device
        '');
    };
}
